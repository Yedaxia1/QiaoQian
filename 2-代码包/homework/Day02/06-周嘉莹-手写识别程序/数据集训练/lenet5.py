...
for e in range(epoch):
    ...
    for x_, y_ in train_loader:
        # 向前预测结果
        y_pred = net(x_.view(-1, 1, 28, 28))   # x_的格式必须是NCHW的格式
        # 使用结果计算误差
        ...
        # with torch.no_grad():
        #     # 损失值的输出
        #     # print(F"轮数{e:03d}/批次{n:02d}，\t损失值：{loss:6.4f}")
        #     # 验证
        #     all_num = 0.0   # 测试集的总样本数
        #     acc = 0.0   # 识别个数
        #     for v_x, v_y in valid_loader:
        #         all_num += len(v_y) # 统计样本数
        ...
        #     print(F"轮数{e:03d}/批次{n:02d}，\t损失值：{loss:6.4f}, \t识别正确率：{100.0 * acc/all_num:6.2f}%")
    with torch.no_grad():
        all_num = 0.0   # 测试集的总样本数
        acc = 0.0   # 识别个数
        for v_x, v_y in valid_loader:
            ...
            # 判别预测类别是否与标签相等
            acc += (cls_pred == v_y).float().sum()
        print(F"轮数{e:03d}，\t损失值：{loss:6.4f}, \t识别正确率：{100.0 * acc/all_num:6.2f}%")
